...
export default function StudentProfileScreen() {
  ...
  // Function to refresh follower/following counts
  const refreshFollowerCounts = React.useCallback(async (userId: string) => {
    try {
  ...
  // Load follower/following counts when student is loaded
  useEffect(() => {
    if (student) {
      const userId = student.userId || student.user?.id;
      if (userId) {
        refreshFollowerCounts(userId);
      }
    }
  ...
  const handleFollow = async () => {
    ...
    try {
      if (isFollowing) {
        ...
        // Refresh follower/following counts after unfollow
        await refreshFollowerCounts(followingId);
      } else {
        ...
        // Refresh follower/following counts after follow
        await refreshFollowerCounts(followingId);
      }
    } catch (error: any) {
    ...
    <Card key={talent.id} style={styles.talentCard}>
      <CardContent style={styles.talentContent}>
          {talent.files && talent.files.length > 0 && (
            ...
            >
              {talent.files.map((file: string, index: number) => {
