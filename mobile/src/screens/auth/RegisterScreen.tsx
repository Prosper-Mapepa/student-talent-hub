...
const RegisterScreen: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'student' | 'faculty' | 'business' | 'alumni'>('student');
  ...
  const validateStudentForm = () => {
    ...
    if (activeTab === 'student' || activeTab === 'alumni') {
      if (!studentData.major) newErrors.major = 'Major is required';
      if (activeTab === 'student' && !studentData.year) newErrors.year = 'Year is required';
      if (activeTab === 'alumni' && !studentData.graduationYear) newErrors.graduationYear = 'Graduation year is required';
    }
  ...
  const handleStudentRegister = async () => {
    ...
    try {
      ...
      // Alumni: send year as ALUMNI and graduationYear (calendar year)
      if (activeTab === 'alumni') {
        payload.year = 'ALUMNI';
        payload.graduationYear = studentData.graduationYear || undefined;
      } else {
  ...
  return (
    <View style={styles.fullScreen}>
      <StatusBar barStyle="light-content" backgroundColor={COLORS.teal700} translucent />
      <LinearGradient
        colors={[...GRADIENTS.screenRich]}
        ...
        <SafeAreaView style={styles.safeArea}>
          ...
          >
            ...
            >
              ...
              <AuthBranding subtitle="Create your VeriTalent account" compact />
              ...
              >
                ...
                >
                  <Text style={[styles.tabText, activeTab === 'alumni' && styles.activeTabText]}>ALUMNI</Text>
                </TouchableOpacity>
              </Animated.View>
              ...
              >
                    ...
                    {activeTab === 'faculty' ? (
                      ...
                      <>
                        <View style={styles.inputContainer}>
                          <TextInput
                            style={styles.input}
                            placeholder={activeTab === 'alumni' ? 'Major / Degree' : 'Major'}
                            placeholderTextColor="#666"
                            value={studentData.major}
                            onChangeText={text => handleStudentChange('major', text)}
                        ...
                        <TouchableOpacity 
                          ...
                          activeOpacity={0.7}
                        >
                          <Text style={[styles.selectText, (activeTab === 'alumni' ? !studentData.graduationYear : !studentData.year) && styles.selectPlaceholder]}>
                            {activeTab === 'alumni'
                              ? (studentData.graduationYear || 'Select Graduation Year')
                              : (studentData.year ? years.find(y => y.value === studentData.year)?.label : 'Select Year')}
                          </Text>
                          <Ionicons name="chevron-down" size={20} color="#666" />
                        </TouchableOpacity>
                        {activeTab === 'alumni' ? (errors.graduationYear && <Text style={styles.fieldError}>{errors.graduationYear}</Text>) : (errors.year && <Text style={styles.fieldError}>{errors.year}</Text>)}

                        {/* Year / Graduation Year Picker Modal */}
                        <Modal
                        ...
                        >
                          <View style={styles.modalOverlay}>
                            <View style={styles.modalContent}>
                              <View style={styles.modalHeader}>
                                <Text style={styles.modalTitle}>{activeTab === 'alumni' ? 'Select Graduation Year' : 'Select Year'}</Text>
                              ...
                              {activeTab === 'alumni' ? (
                                <Picker
                                  selectedValue={studentData.graduationYear}
                                  onValueChange={(value: string) => {
                                    handleStudentChange('graduationYear', value);
                ...
                <View style={styles.termsRow}>
                  ...
                  >
                    <Ionicons name={agreedToTerms ? 'checkbox' : 'square-outline'} size={20} color={COLORS.teal700} />
                  </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  tabContainer: {
    ...
    paddingTop: 8,
    paddingHorizontal: 8,
    marginBottom: isTablet ? (isLargeTablet ? 30 : 25) : 5,
    borderWidth: 1,
  ...
  tab: {
    width: '48%',
    paddingVertical: isTablet ? (isLargeTablet ? 16 : 14) : 12,
    paddingHorizontal: 8,
    marginBottom: 8,
    borderRadius: 14,
    alignItems: 'center',
  ...
  tabText: {
    fontSize: isTablet ? (isLargeTablet ? 15 : 14) : 12,
    fontWeight: '700',
