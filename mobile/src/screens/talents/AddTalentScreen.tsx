...
import { fetchAllTalents, fetchStudentTalents, addTalent } from '../../store/slices/talentsSlice';
...
export default function AddTalentScreen() {
  ...
  const { isLoading, error, talents } = useAppSelector((state) => state.talents);
  const hasStudentPro = useAppSelector(selectHasStudentPro);

  const myTalentCount = talents.length;
  ...
  const handleSubmit = async () => {
    ...
    if (!hasStudentPro && myTalentCount >= FREE_TALENT_LIMIT) {
      Alert.alert(
        'Upgrade to Student Pro',
        `Free accounts can showcase up to ${FREE_TALENT_LIMIT} talents. Upgrade for unlimited showcases.`,
        [
    ...
    <ScrollView style={styles.container}>
      {/* <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
          <Ionicons name="arrow-back" size={24} color="#0f6e56" />
        </TouchableOpacity>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.uploadButtons}>
              <TouchableOpacity style={styles.uploadButton} onPress={pickImage}>
                <Ionicons name="image" size={20} color="#0f6e56" />
